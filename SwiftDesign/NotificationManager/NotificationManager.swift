...
func startHourlyNotifications() {
    ...
    let content = UNMutableNotificationContent()
    content.title = "정기 알림"
    content.body = "매시간 30분에 보내는 알림입니다."
    content.sound = UNNotificationSound.default
    
    // 매시간 30분에 알림을 보내기 위한 트리거 설정
...
//func fetchAPIData(completion: @escaping (String) -> Void) {
//    let url = URL(string: "https://www.naver.com")! // 실제 API URL로 교체
//    let task = URLSession.shared.dataTask(with: url) { data, response, error in
//        if let error = error {
//            print("API 호출 에러: \(error.localizedDescription)")
//            completion("API 호출 실패")
//            return
//        }
...
//        
//        if minute == 5 || minute == 7 || minute == 8 || minute == 9 || minute == 16 || minute == 21 || minute == 29 || minute == 36 || minute == 35 || minute == 34 || minute == 28 || minute == 27 {
//            fetchApiData { result in
